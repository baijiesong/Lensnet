...
class Unet(nn.Module):
    def __init__(self, args: "tupperware", in_c: int = 4):
        ...
        # 在上采样阶段添加小波变换模块
        self.wavelet_up3 = LWN(
            dim=128,  # 与conv_up3输出通道匹配
            wavelet='haar',
            initialize=True,
            use_ca=True,
            use_sa=True
        ...
        self.wavelet_up2 = LWN(
            dim=64,   # 与conv_up2输出通道匹配
            wavelet='haar', 
            initialize=True,
            use_ca=True,
            use_sa=True
        ...
        self.wavelet_up1 = LWN(
            dim=32,   # 与conv_up1输出通道匹配
            wavelet='haar',
            initialize=True,
            use_ca=True,
            use_sa=True
        ...
        self.wavelet_up0 = LWN(
            dim=32,   # 与conv_up0输出通道匹配
            wavelet='haar',
            initialize=True,
            use_ca=True,
            use_sa=True
    ...
    def forward(self, img):
        ...
        wave_feat0 = self.wavelet0(layer0)
        layer0 = layer0 + wave_feat0

        # 第二层wavelet特征融合
        layer1 = self.layer1(layer0)
        wave_feat1 = self.wavelet1(layer1)
        layer1 = layer1 + wave_feat1

        # 第三层wavelet特征融合
        layer2 = self.layer2(layer1)
        wave_feat2 = self.wavelet2(layer2)
        layer2 = layer2 + wave_feat2

        # 第四层wavelet特征融合
        layer3 = self.layer3(layer2)
        wave_feat3 = self.wavelet3(layer3)
        layer3 = layer3 + wave_feat3
...
def main(_run):
    ...
    height = 256
    width = 256
    input_channels = channels * (args.pixelshuffle_ratio ** 2)
    test_input = torch.randn(batch_size, input_channels, height, width).to(args.device)
    is_admm = "admm" in args.exp_name
    in_c = 3 if is_admm else 4

    
    # 初始化模型
    model = Unet(args, in_c=in_c).to(args.device)
